...
copy build\MidiPlayer.exe release\JMPlayer_R2.4d.exe
...
cd /D %~dp0release
windeployqt JMPlayer_R2.4d.exe --release --no-translations --no-opengl-sw
...
echo Executable: .\release\JMPlayer_R2.4d.exe
echo Icon: .\release\K_icon.ico
echo.

if exist release\JMPlayer_R2.4d.exe (
    for %%A in (release\JMPlayer_R2.4d.exe) do (
