...
copy "%BUILD_DIR%\MidiPlayer.exe" "%RELEASE_DIR%\JMPlayer_R2.7c.exe"
...
cd /d "%RELEASE_DIR%"
windeployqt JMPlayer_R2.7c.exe --release --no-translations --no-opengl-sw
...
echo Executable: %RELEASE_DIR%\JMPlayer_R2.7c.exe
echo.

if exist "%RELEASE_DIR%\JMPlayer_R2.7c.exe" (
    for %%A in ("%RELEASE_DIR%\JMPlayer_R2.7c.exe") do (
